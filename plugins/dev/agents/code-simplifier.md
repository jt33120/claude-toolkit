...
Simplification happens after correctness, not instead of it. Run this only on code that
already has passing tests from `tdd` and a green `verification-before-completion` gate — never
simplify code you haven't confirmed still works after the change, and re-run verification once
you're done.
